...
(function() {
    ...
    $$.Native = {
        ...
        openNativeBank: function(options) {
            $$.platformAdapter.call('openNativeBank', options.queryString, options.callback ||
            function() {});
        },
        /**
         * 发送HTTP请求
        ...
        requestThird: function(options) {
            ...
            $$.platformAdapter.call('requestThird', options.url, type, data, options.success, options.error);
        },
        ...
        getData: function(options) {
            $$.platformAdapter.call('getData', options.name,
            function(value) {
                try {
        ...
        getLocalData: function(options) {
            $$.platformAdapter.call('getLocalData', options.name,
            function(value) {
                try {
        ...
        requestImage: function(options) {
            $$.platformAdapter.call('requestImage', options.url,
            function(value) {
                options.callback(value);
            });
        },
        ...
        */
        ...
        payByPingAnFu: function(options) {
            var callback = options.callback || defaultCallback;
            $$.platformAdapter.call("payByPingAnFu", options.tranCode, callback, options.params);
        },
        /**
          * getHealthyWalkData 健步行接入
         **/
        // getHealthyWalkData: function (callback) {
        //     $$.platformAdapter.call("getHealthyWalkData", callback);
        // },

        getHealthyWalkData: function(options) {
            $$.platformAdapter.call("getHealthyWalkData", options.flag, options.callback);
        },
        /**
          * copyToPasteboard 调用粘贴板
         **/
        copyToPasteboard: function(option) {
            $$.platformAdapter.call("copyToPasteboard", option.content);
        },
        /**
          * copyToPasteboard 调用粘贴板
         **/
        ...
        selectDate: function(options) {
            ...
            $$.platformAdapter.call("selectDate", formatStr, yearStart, yearEnd, currentDate, leftBtnCB, rightBtnCB);
        },
        /**
         * 显示日期组件
         * 格式：y, y-m, m-d, y-m-d ,H:M
         */
        selectDateStyle: function(options) {
            var options = options || {},
            defaultDate = options.defaultDate || '',
            leftBtnCB = options.leftBtnCallback || defaultCallback,
            rightBtnCB = options.rightBtnCallback || defaultCallback;
            $$.platformAdapter.call("selectDateStyle", defaultDate, leftBtnCB, rightBtnCB);
        },
        /*
         * 调用地图
         *
         */
        callAMapServerWithData: function(jsonString) {
            $$.platformAdapter.call("callAMapServerWithData", jsonString, defaultCallback);
        },
        /**
         * ç¤¾äº¤å¹³å°åäº«
         * @param options
         *
         */
        shareImageURL: function(options) {

            $$.platformAdapter.call("shareImageURL", options.imageURL, options.title, options.content, options.url, options.callback);
        },
        uploadPhoto: function(options) {
            $$.platformAdapter.call("uploadPhoto", options.path, options.callback);
        },

        /**
        *   手势密码
        **/
        settingSwipLock: function(options) {
            $$.platformAdapter.call("settingSwipLock", options.directory, options.param, options.callback);
        },

        /**
         * 设置参保人的年龄和性别
         */
        setAgeAndSex: function(options) {
            var options = options || {},
            personAge = options.personAge,
            personSex = options.personSex,
            leftBtnCB = options.leftBtnCallback || defaultCallback,
            rightBtnCB = options.rightBtnCallback || defaultCallback;
            $$.platformAdapter.call("selectAgeAndSex", personAge, personSex, leftBtnCB, rightBtnCB);
        },
        /**
         * 获取参保人的年龄和性别
         */
        getAgeAndSex: function(options) {
            var options = options || {},
            rightBtnCB = options.rightBtnCallback || defaultCallback;
            $$.platformAdapter.call("getUserAgeAndSexWithCallBack", rightBtnCB);
        },
        /**
         * 调转好医生
         */
        jumpGoodDoctorWithCallBack: function() {
            $$.platformAdapter.call("jumpGoodDoctorWithCallBack");
        },
        /**
         * android返回键
         * @param {Object} options
         */
        selectPhotoCounts: function(options) {
            var options = options || {},
            photoCounts = options.photoCounts,
            cancelCallback = options.cancelCallback,
            confirmCallback = options.confirmCallback;
            $$.platformAdapter.call("selectPhotoCounts", photoCounts, cancelCallback, confirmCallback);
        },
        /**
         * 意见反馈选择照片
         * @param {Object} options
         */
        choosePhotoCounts: function(options) {
            var options = options || {},
            photoCounts = options.photoCounts,
            cancelCallback = options.cancelCallback,
            confirmCallback = options.confirmCallback,
            functionSign = options.functionSign;
            $$.platformAdapter.call("choosePhotoCounts", photoCounts, cancelCallback, confirmCallback, functionSign);
        },
        /**
         * 意见反馈提交照片（但是H5只用传问题模块和解答的string）
         * @param {Object} options
         */
        upLoadOpinion: function(options) {
            var options = options || {},
            questionModule = options.questionModule,
            questionDescribe = options.questionDescribe,
            callback = options.callback;
            $$.platformAdapter.call("upLoadOpinion", questionModule, questionDescribe, callback);
        },

        saveCurrentPageMessage: function(options) {
            var options = options || {},
            moduleName = options.moduleName,
            currentPage = options.currentPage;
            $$.platformAdapter.call("saveCurrentPageMessage", moduleName, currentPage);
        },
        selectPhotoWithCallBack: function(options) {
            var options = options || {},
            cancelCallback = options.cancelCallback,
            confirmCallback = options.confirmCallback;
            $$.platformAdapter.call("selectPhotoWithCallBack", cancelCallback, confirmCallback);
        },
        viewPhotoByType: function(options) {
            var options = options || {},
            mainPaths = options.mainPaths,
            index = options.index,
            confirmCallback = options.confirmCallback;
            $$.platformAdapter.call("viewPhotoByType", mainPaths, index, confirmCallback);
        },
        getNativeUser: function(options) {
            var options = options || {},
            param = options.param,
            confirmCallback = options.confirmCallback;
            $$.platformAdapter.call("getLocalData", param, confirmCallback);
        },
        deleteSymptomWithName: function(imgName) {
            $$.platformAdapter.call("deleteSymptomWithName", imgName);
        },
        clearSymptomWithCallback: function() {
            $$.platformAdapter.call("clearSymptomWithCallback");
        },
        uploadNoHud: function(options) {
            var data = options.data,
            param = [],
            type = options.type;
            if (!/^get|post$/.test(type)) {
            ...
            if (options.sign === 'SymptomSign') {
                $$.platformAdapter.call("uploadNoHud", options.url, options.sign, data, options.success, options.error);
            } else if (options.sign === 'InjurySign') {
                $$.platformAdapter.call("uploadJobCompensation", options.url, options.sign, data, options.success, options.error);
            }
            //      		$$.platformAdapter.call("uploadNoHud",options.url,options.sign,data,options.success,options.error);
        },
        uploadImages: function(options) {
            ...
            $$.platformAdapter.call('uploadImages', options.url, type, data, options.success, options.error);
        },
        jumpToHealthyYun: function() {
            $$.platformAdapter.call("jumpToHealthyYun", defaultCallback);
        },
        // 获取当前城市
        getCityName: function(callback) {
            $$.platformAdapter.call('getCityName', callback);
        },
        // 接入健步行
        jumpToHealthyWalk: function() {
            $$.platformAdapter.call("jumpToHealthyWalk", defaultCallback);
        },
        ...
        * 绍兴市民卡 调用地图
        **/
        callAMapServerForShaoxing: function(options) {
            $$.platformAdapter.call("callAMapServerForShaoxing", options.jsonString, options.defaultCallback);
        },
        /*
        * 绍兴市民卡 照片上传
        **/
        uploadCityCardPhoto: function(options) {
            $$.platformAdapter.call("uploadCityCardPhoto", options.path, options.callback);
        },
        /*
        * 绍兴市民卡 获取 姓名 身份证号 社保卡号
        **/
        getPersonalInfo: function(options) {
            $$.platformAdapter.call("getPersonalInfo", options.flag, options.callback);
        },
        /**
         * 获取定位经纬度 wangchao798
         */
        getCurrentLocation: function(options) {
            var callback = options.callback || defaultCallback;
            $$.platformAdapter.call("getCurrentLocation", callback);
        },
        jumpToFollowUpWithTime: function(options) {
            var callback = options.callback || defaultCallback;
            $$.platformAdapter.call("jumpToFollowUpWithTime", options.time, options.sex, options.idCard, options.patientName, options.chronicVisitId, options.disease, options.visitContent, callback);
        },
        selectDateStyle: function(options) {
            var options = options || {},
            defaultDate = options.defaultDate,
            ...
            cancelCallback = options.cancelCallback,
            confirmCallback = options.confirmCallback;
            $$.platformAdapter.call("selectDateStyle", defaultDate, title, cancelCallback, confirmCallback);
        },
        /**
        * native事件分发函数
        * @param options.functionName::: 告诉native需要调用哪一个方法的方法名,注意此方法的参数是不确定的，需要几个参数，加几个冒号，根据需求而定
        *        options.params 此方法名所需要的参数数组
        *        options.callback 默认回调
        */
        interectWithNative: function(options) {
            $$.platformAdapter.call("interectWithH5", options.functionName, options.params, options.callback);
        },
    };
})();
