...
var zip = require('gulp-zip'); // gulp-zip zip pkg
...
var args = require('yargs')
    ...
    .alias('b', 'build')
    .alias('r', 'run')
    .alias('c', 'core') // zip core
    .alias('z', 'zip')  // zip www
    .alias('x', 'del')  // del dist
...
var zipApp = args.zip,   // zip www
    zipCore = args.core,
    delDist = args.del; // zip core
...
/**
 * 清空dist
 * @param  {[type]} 'deleteDist' [description]
 * @param  {[type]} function     (done         [description]
 * @return {[type]}              [description]
 */
gulp.task('deleteDist', function (done) {
    del([distDir], done);
});
/**
 * 压缩Core => core.zip PkG
...
gulp.task('core', function () {
    return gulp.src('core/**')
        .pipe(zip('core_' + getDateTime() + '.zip'))
        .pipe(gulp.dest('dist'))
...
gulp.task('zip', function () {
    return gulp.src('www/**')
        .pipe(zip('cmsCommon_' + getDateTime() + '.zip'))
        .pipe(gulp.dest('dist'))
        .on('error', errorHandler);
});
...
gulp.task('seed', function() {
    var seedFiles = require('./seed.json');
    var distDir = path.resolve('./core');

    del([distDir]);

    return gulp.src(seedFiles)
        .pipe(plugins.concat('seed.js'))
        .pipe(plugins.if(build, plugins.uglify()))
        .pipe(gulp.dest(build ? distDir : targetDir))
        // .pipe(gulp.dest(distDir))
        // .pipe(plugins.if(!build, gulp.dest(targetDir)))
        .on('error', errorHandler);
});


gulp.task('injectHTML', ['seed', 'jsHint', 'scripts'], function() {
  ...
  return gulp.src('app/index.html')
    ...
    // inject seed.JS
    .pipe(_inject(gulp.src(build ? '../core/seed.js' : 'seed.js', {
        cwd: targetDir
    }), 'seed'))
...
gulp.task('watchers', function() {
    ...
    gulp.watch('./seed.json', ['seed']);
    gulp.watch('app/styles/**/*.scss', ['styles']);
    gulp.watch('app/fonts/**', ['fonts']);
    gulp.watch('app/icons/**', ['iconfont']);
    gulp.watch('app/images/**', ['images']);
    gulp.watch('app/scripts/**/*.js', ['injectHTML']);
    gulp.watch('app/templates/**/*.html', ['injectHTML']);
    gulp.watch('app/index.html', ['injectHTML']);
